...
import type { IMailerService } from 'src/mailer/mailer'
...
export class MailsService implements IMailsService {
  ...
  async confirmRegisterUser(mailData: MailData<{ hash: string; user: string }>): Promise<void> {
    await this.mailerService.sendMail({
      ...
      context: {
        ...
        confirmationLink: `${this.configService.get<string>('app.frontendDomain', {
          infer: true
        })}/confirm-email/${mailData.data.hash}`
      }
    })
  ...
  async forgotPassword(mailData: MailData<{ hash: string; user: string }>): Promise<void> {
    await this.mailerService.sendMail({
      to: mailData.to,
